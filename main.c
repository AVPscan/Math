...
#define T 2
anu la = T, cl = 0, a[] = {0,0,0,0,0,0,0,0}, lb = T, b[] = {0,128,128,0,0,0,0,0,0};

void Num(anu s, an a) { anu t[8]; printf("(%d) ", s + 1); if (s > 7) { printf("XxX "); return; } Mat.L = s;
  if (Mat.Be) { FSWAP((an)t, a, s, 0); } else { FMOV((an)t, a, s, 0); } s = Mat.L; a = (an)t; FCOLD(a, a, s, 0); s = Mat.l;
