...
int main(void) { buf r, e; Dbuf rm; zbuf zr, ze; zDbuf zm; Mat.L = Lon; Mat.Nim = 0; Mat.A = (anu*)&a.h; Mat.B = (anu*)&b.h;
  ...
  printf("Long = %d\n", Lon);
  FADD(Mat.L, (anu*)&r, Mat.A, Mat.B); Show(1, "+", &r, &e);
  FSUB(Mat.L, (anu*)&r, Mat.A, Mat.B); Show(1, "-", &r, &e);
  FMUL(Mat.L, (anu*)&rm, Mat.A, Mat.B); Show(2, "*", &rm, &e); 
  FDIV(Mat.L, (anu*)&r, Mat.A, Mat.B, (anu*)&e); Show(1, "/", &r, &e); Mat.Nim = 1;
  FADD(Mat.L, (anu*)&zr, Mat.A, Mat.B); Show(1, "+", &zr, &ze);
  FSUB(Mat.L, (anu*)&zr, Mat.A, Mat.B); Show(1, "-", &zr, &ze);
  FMUL(Mat.L, (anu*)&zm, Mat.A, Mat.B); Show(2, "*", &zm, &ze);
  FDIV(Mat.L, (anu*)&zr, Mat.A, Mat.B, (anu*)&ze); Show(1, "/", &zr, &ze); return 0; }
