...
#define T 3
anu a[] = {0,0,2,0,0,0,0,0}, b[] = {255,255,254,0,0,0,0,0,0}, c[16], r[16], e[8];
...
void Num(anu s, an a) { printf("(%d) ", s + 1); if (s > 7) { printf("XxX "); return; }
  ...
  if (Mat.Nim) { if (s > 3) printf("%ld " , *(uint64_t*)a); else if (s > 1) printf("%d ", *(uint32_t*)a);
...
void Show(char s) { anu i = 4; printf("\nA%cB\n", s); while(i--) {
    Mat.Nim = ((3 - i) & 2) ? 1 : 0; Mat.V = ((3 - i) & 1) ? 1 : 0; if (s == 'a') { Si(); Fadd(Mat.lb, Mat.R, Mat.A, Mat.B); Se(); }
    else if (s == '+') { FADD(Mat.lb + 1, Mat.R, Mat.A, Mat.B); } else if (s == '-') { FSUB(Mat.lb + 1, Mat.R, Mat.A, Mat.B); }
    else if (s == '*') { FMUL(Mat.lb + 1, Mat.R, Mat.A, Mat.B); } else if (s == '/') { FDIV(Mat.lb + 1, Mat.R, Mat.A, Mat.B, Mat.RE); }
