...
anu la = 0, a[] = {127,255,255,3,4,5,6,7}, l = 8;
anu lb = 0, b[] = {255,255,255,255,255,255,255,255};
...
void Show(char s) { anu i = 4; printf("\nA%cB\n", s);
  while(i--) { Mat.Nim = ((3 - i) & 2) ? 1 : 0; Mat.V = ((3 - i) & 1) ? 1 : 0; Mat.C = 0; Flong(la, lb) 
	...
	else if (s == '/') FDiv(Mat.R, Mat.E) else if (s == 'c') FAddc(Mat.R,1)
    printf("%c%c%c%c ", Mat.Nim ? 'N':' ', Mat.V ? 'V':' ', Mat.C ? 'C':' ', Mat.F ? (Mat.N ? 'I':'Z'):(Mat.N ? '-':'+'));
    Num(Mat.F, Mat.lar, Mat.R); Num(0, la, Mat.A); if (s != 'c') Num(0, lb, Mat.B); else printf(" X|x  Const");
    if (s == '/') { printf("  %c ", Mat.Fe ? (Mat.Ne ? 'I':'Z') : (Mat.Ne ? '-':'+')); Num(Mat.Fe, Mat.lbe, Mat.E); } printf("\n"); } }
...
int main(void) { anu r[l + l], e[l]; Fini(OutputBigEndian) Faddr((As)r, (As)e, (As)a, (As)b)
  ...
  Show('+'); Show('c');/*Show('-'); Show('*'); Show('/');*/return 0; }
