...
void Num(anu s, an a) { if (s > 7) { printf("XxX"); return; }
  printf("(%d) ", s + 1); Fswap(s, (an)c, a); a = (an)c; FCold(s, a, a); s = Mat.lre;
...
void Show(char s) { anu i = 4; printf("\nA%cB\n", s); while(i--) { Mat.Nim = ((3 - i) & 2) ? 1 : 0; Mat.V = ((3 - i) & 1) ? 1 : 0;
    if (s == 'a') Fadd(Mat.lb, Mat.R, Mat.A, Mat.B); else if (s == '+') FADD(Mat.lb + 1, Mat.R, Mat.A, Mat.B);
    else if (s == '-') FSUB(Mat.lb + 1, Mat.R, Mat.A, Mat.B); else if (s == '*') FMUL(Mat.lb + 1, Mat.R, Mat.A, Mat.B);
