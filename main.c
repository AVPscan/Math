...
anu la = 0, a[] = {127,0,0,0,0,0,0,0}, l = 8;
anu lb = 0, b[] = {127,255,255,255,255,255,255,255};

void Num(anu f, anu s, an a) { anu t[l]; if (s >= l) { printf("(%d) X ", s + 1); return; }
  FMOV(s, (an)t, a); a = (an)t; printf("(%d) ", Mat.lar + 1); Fcold(Mat.lar, a) if (f == 2) { printf("inf "); return; }
...
void Show(char s) { anu i = 4; printf("\nA%cB\n", s);
  while(i--) { Mat.Nim = ((3 - i) & 2) ? 1 : 0; Mat.V = ((3 - i) & 1) ? 1 : 0; Mat.C = 0; Flong(la, lb) 
    ...
    Num(Mat.F, Mat.lar, Mat.R); Num(0, la, Mat.A); Num(0, lb, Mat.B); if (s == '/') {
	  printf("tile %c ", Mat.Fe ? (Mat.Ne ? 'I':'Z') : (Mat.Ne ? '-':'+')); Num(Mat.Fe, Mat.lbe, Mat.E); }
    printf("\n"); } }

int main(void) { anu r[l + l], e[l]; Fini(OutputBigEndian) Faddr((As)r, (As)e, (As)a, (As)b)
  if (InputBigEndian) { Fswap(lb, Mat.B) Fswap(la, Mat.A) } printf("(%d) A (%d) B", la + 1, lb + 1);
