...
void Show(anu i, const char *str, void *r, void *e) {
  printf(" %s%s", (Mat.C) ? "C" : " ", (Mat.F) ? (Mat.Z) ? "I " : "Z " : (Mat.Z) ? "- " : "+ ");
  Mat.C = 0; i = (i != 2) ? 1 : 2; if (!BE) { FSWAP(Mat.L * i, (anu*)r, (anu*)r); FSWAP(Mat.L, (anu*)e, (anu*)e); }
#if Lon == 4
  ...
  if (str[0] == '*') { printf(" long %d", ((Mat.Lh << 8) | Mat.Ll)); }
    ...
    printf(" %s", (Mat.Fe) ? (Mat.Ze) ? " I" : " Z" : (Mat.Ze) ? " -" : " +"); } printf("\n"); }

int main(void) {
  printf("Long = %d\n", Lon); buf r, e; Dbuf rm; zbuf zr, ze; zDbuf zm; Mat.L = Lon; Mat.A = (anu*)&a; Mat.B = (anu*)&b;
  FSWAP(Mat.L, (anu*)&o1, Mat.A); FSWAP(Mat.L, (anu*)&o2, Mat.B); FSWAP(Mat.L, (anu*)&o3, (anu*)&a); FSWAP(Mat.L, (anu*)&o4, (anu*)&b);
  if (BE) { FSWAP(Mat.L, (anu*)&o1, (anu*)&o1); FSWAP(Mat.L, (anu*)&o2, (anu*)&o2); FSWAP(Mat.L, (anu*)&o3, (anu*)&o3);
    FSWAP(Mat.L, (anu*)&o4, (anu*)&o4); }
  Mat.Nim = 0; FADD(Mat.L, (anu*)&r, Mat.A, Mat.B); Show(1, "+", &r, &e); FSUB(Mat.L, (anu*)&r, Mat.A, Mat.B); Show(1, "-", &r, &e);
  FMUL(Mat.L, (anu*)&rm, Mat.A, Mat.B); Show(2, "*", &rm, &e); Mat.Lh = 0; Mat.Ll = Mat.L; FDIV(Mat.L, (anu*)&r, Mat.A, Mat.B, (anu*)&e); Show(1, "/", &r, &e);
  Mat.Nim = 1; FADD(Mat.L, (anu*)&zr, Mat.A, Mat.B); Show(1, "+", &zr, &ze); FSUB(Mat.L, (anu*)&zr, Mat.A, Mat.B); Show(1, "-", &zr, &ze);
  FMUL(Mat.L, (anu*)&zm, Mat.A, Mat.B); Show(2, "*", &zm, &ze); Mat.Lh = 0; Mat.Ll = Mat.L; FDIV(Mat.L, (anu*)&zr, Mat.A, Mat.B, (anu*)&ze); Show(1, "/", &zr, &ze);
  return 0; }
