...
anu a[] = {251,5,0,0}, la = 0, b[] = {6,0,0,0}, lb = 0, cl = 0, Nim = 0, Be = 0, V = 0, l, c[8], r[512], e[256];

void Num(anu s, an a) {
  if (s > 7) { printf("XxX"); return; } printf("(%d) ", s + 1); FCold(s, (an)c, a); a = (an)c; s = Mat.lre;
  if (Mat.Be == 0) Fswap(s, a, a);      // Так как задаю операнды в Be формате и библиотека пока на нём (длина умножения! странно)
...
void Show(const char *s) {
  ...
  printf("\n"); Mat.C = 0; Flong(cl, la); }
void Test(anu type) {
  Mat.Nim = type; FADD(lb, Mat.R, Mat.A, Mat.B); Show("+"); FSUB(lb, Mat.R, Mat.A, Mat.B); Show("-");
  FMUL(lb, Mat.R, Mat.A, Mat.B); Show("*"); FDIV(lb, Mat.R, Mat.A, Mat.B, Mat.RE); Show("/"); }
