...
void FLVD(an r, anu Dl, anu Dh) { Mat.lar = 1; *r++ = Dl; Mat.N = Mat.Nim ? (Dh & 0x80) ? 0xFF:0:0;
  ...
  if (Mat.lar && Dh == Mat.N && !(Mat.Nim && ((Dl ^ Mat.N) & 0x80))) Mat.lar--; }
void FMOV(anu l, an r, an a) { Mat.lar = l; Mat.e = r; Mat.dr = 0;
  ...
  else { l = Mat.lar; while(Mat.lar && *r == Mat.N && !(Mat.Nim && ((*(r - 1) ^ Mat.N) & 0x80))) { Mat.lar--; r--; }
...
void FADD(an r, an a, anu l, an b) { Mat.lbe = l; Mat.C = (Mat.C != 0); Mat.da = 0; Mat.db = 0; Mat.r = 0;
  Mat.e = r; if (Mat.lar < l) { Mat.a = b; b = a; a = Mat.a; l = Mat.lar; Mat.lar = Mat.lbe; } Mat.N = Mat.lar - l;
  ...
  *r = l; Mat.C = ((Mat.Nim ? (((Mat.na & 0x80) ^ (l & 0x80)) || (l == 0x80 && !Mat.dr)) : Mat.C) != 0);
  ...
  if ((Mat.F = (Mat.dr || *r) ? (Mat.Nim && !Mat.dr && (*r == 0x80)) ? 2:0:1)) { *Mat.e = l; Mat.lar = 0;
	Mat.C = (Mat.F == 2) ? 1:Mat.C; return; }
  l = Mat.lar; while(Mat.lar && *r == Mat.N && !(Mat.Nim && ((*(r - 1) ^ Mat.N) & 0x80))) { Mat.lar--; r--; }
  if (Mat.Nim) { if ((l - Mat.lar) && *r == 0x80) { l = Mat.lar; while(l && *--r == 0) { l--; } Mat.lar += !l; }
	if (Mat.na == Mat.nb && Mat.N != Mat.na) { *Mat.e = 0xFF; Mat.lar = 0; Mat.F = 2; Mat.N = 0xFF; Mat.C = 1; } } }
