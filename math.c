...
void FADD(an r, an a, anu l, an b) { Mat.N = l; Mat.r = r; Mat.a = a; Mat.b = b; Mat.lb = Mat.N;
  ...
  if (Mat.N) do { Mat.x = *Mat.e++; Mat.y = (*Mat.r++ = Mat.na + Mat.x + Mat.C);
  ...
  if (Mat.C && Mat.l != 255) { Mat.l++; Mat.j |= Mat.y; *++Mat.r = Mat.Nim ? Mat.N:Mat.C; Mat.C--; }
  Mat.N = (Mat.Nim && (*Mat.r & 128)) ? 255:0; if ((Mat.F = !(Mat.j | *Mat.r))) Mat.l = 0; }
...
void FMUL(an r, an a, anu l, an b) { Mat.N = l; Mat.r = r; Mat.a = a; Mat.b = b; Mat.lb = Mat.N;
  ...
  Mat.l = Mat.k; Mat.i -= Mat.F; Mat.j -= Mat.N; Mat.i += Mat.j; Mat.e = &Mat.H.l; Mat.ae = &Mat.L.l;
  ...
  Mat.ae = Mat.r; *Mat.ae++ = Mat.C; Mat.j = Mat.l; do *Mat.ae++ = 0; while(Mat.j--); Mat.ae = &Mat.H.l;
  do { if ((Mat.i = *Mat.ae++)) {
         Mat.a = &Mat.S.l; Mat.be = &Mat.L.l; Mat.j = Mat.N; do *Mat.a++ = *Mat.be++; while(Mat.j--);
         do { Mat.t = 0; Mat.be = &Mat.S.l; Mat.j = Mat.N;
