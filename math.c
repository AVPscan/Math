...
void FSWAP (anu l, anu *r, anu *a) {
  if (l) { Mat.r = r + l; Mat.a = a + l; Mat.fa = (l & 1); l >>= 1; while(l--) {
    Mat.ba = *--Mat.a; Mat.bb = *a++; *r++ = Mat.ba; *--Mat.r = Mat.bb; } if (Mat.fa) { *r = *a; } } }

void FMOV (anu l, anu *r, anu *a) {
  while(l--) *r++ = *a++; }

void FLDA (anu l, anu *r, anu D) {
  Mat.Carry = 0; if (l) { Mat.Rnim = (Mat.Nim) ? (D & 0x80) ? 0xFF : 0 : 0;
  Mat.Riz = (D) ? (Mat.Nim && D == 0x80) ? 1 : 0 : 1; while(--l) { *r++ = Mat.Rnim; } *r = D; } }

void FLDVA (anu l, anu *r, anu Dh, anu Dl) {
  Mat.Carry = 0; if (l) { if (--l) { Mat.Rnim = (Mat.Nim) ? (Dh & 0x80) ? 0xFF : 0 : 0;
...
void FRR (anu l, anu *r) {
  r--; while(l--) { Mat.za = *++r & 1; *r >>= 1; if (Mat.Carry) { *r |= 0x80; } Mat.Carry = Mat.za; } Mat.Carry = (Mat.Carry != 0); }

void FRL (anu l, anu *r) {
  r += l; while(l--) { Mat.za = *--r & 0x80; *r <<= 1; if (Mat.Carry) { *r |= 1; } Mat.Carry = Mat.za; } Mat.Carry = (Mat.Carry != 0); }

void VIKARA (anu lr, anu la, anu *r, anu *a) {
  Mat.Carry = 0; if ((Mat.loop = la)) { Mat.Rnim = (Mat.Nim) ? (*a & 0x80) ? 0xFF : 0 : 0; Mat.a = a;
    if ((Mat.Riz = (*a) ? (Mat.Nim && *a == 0x80) ? 1 : 0 : 1)) while(--Mat.loop && (Mat.Riz = (*++Mat.a) ? 0 : Mat.Riz));
...
void FADD (anu l, anu *r, anu *a, anu *b) {
  if ((Mat.loop = l)) { Mat.Riz = 0; Mat.Rnim = 0; Mat.Carry = (Mat.Carry != 0); Mat.zr = !((*a ^ *b) & 0x80); Mat.br = 0;
...
void FMUL (anu l, anu *r, anu *a, anu *b) {
  if (l) { Mat.fa = 0; Mat.fb = 0; Mat.ba = 1; Mat.bb = 1; Mat.Riz = 0; Mat.za = (Mat.Nim && *a & 0x80) ? 0xFF : 0;
    Mat.zb = (Mat.Nim && *b & 0x80) ? 0xFF : 0; Mat.Rnim = Mat.za ^ Mat.zb; Mat.r = r; r += l; Mat.e = r + l; a += l; b += l;
    Mat.a = &Mat.Ho.h; Mat.b = &Mat.Lo.h; Mat.Ll = l; Mat.Lh = ((Mat.Ll += l) < l);
...
void FDIV (anu l, anu *r, anu *a, anu *b, anu *e) {
  if ((Mat.loop = l)) { Mat.fa = 0; Mat.fb = 0; Mat.Riz = 0; Mat.za = (Mat.Nim && (*a & 0x80)) ? 0xFF : 0;
    Mat.zb = (Mat.Nim && (*b & 0x80)) ? 0xFF : 0; Mat.Rnim = Mat.za ^ Mat.zb; r += l; a += l; b += l; e += l;
    Mat.b = &Mat.Lo.h; Mat.ba = 1; Mat.bb = 1;
    do { Mat.fa = (*--a) ? (Mat.fa) ? Mat.fa : l : Mat.fa; Mat.fb = (*--b) ? (Mat.fb) ? Mat.fb : l : Mat.fb;
