...
#define Fmov(l, cl) FMOV(Mat.R, Mat.A, l)
void FSWAP(an r, an a, anu l);                              // Зеркалирование относительно центра числа атомов внутри
#define Fswap(l, cl) FSWAP(Mat.R, Mat.A, l)
void FVIKARA(an r, an a, anu l);                            // Модификация изменение длин чисел
#define Fvikara(l, cl) FVIKARA(Mat.R, Mat.A, l)
void FCOLD(an r, an a, anu l);                              // Приведение к старым форматам l = 1, 2, 4, 8, 16, 32, 64, 128, 256 байт 
#define Fcold(l, cl) FCOLD(Mat.R, Mat.A, l)
...
#define Flong(...) _FInit(0,2,&Mat.Be,_anu(__VA_ARGS__))    // {lb{,la}} (Mat.l = lb; Mat.L = la;)
