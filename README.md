...
typedef uint8_t anu;                                            // 1   anu [0..FF]        	        [1..8] 10^2
//typedef int8_t nanu;                                           //1  n    [0,+1..+7F,80,-7F..-1]
typedef struct { anu l; union {anu m[1]; anu h;}; } vanu;       // 2  v    [0..FFFF]      	        [1..16] 10^4
//typedef struct { anu l; union {anu m[1]; nanu h;}; } vnanu;    //2 vn    [0,+1..+7FFF,8000,-7FFF..-1]
//typedef struct { anu l, m[1], h; };                            //3       [0..FFFFFF]    	      + [17..24] 10^7
//typedef struct { anu l, m[1] nanu h; };                        //3       [0,+1..+7FFFFF,800000,-7FFFFF..-1]
typedef struct { anu l, m[2], h; } an;                          // 4   an  [0..FFFFFFFF]  	        [1..32] 10^9
//typedef struct { anu l, m[2]; nanu h; } nan;                   //4  n    [0,+1..+7FFFFFFF,80000000,-7FFFFFFF..-1]
//typedef struct { anu l, m[3..5], h; };                         //5..7                         +++ [33..56] 10^12, 10^14, 10^16
//typedef struct { anu l, m[3..5] nanu h; };                     //5..7n
typedef struct { anu l, m[6], h; } van;                         // 8  v    [0..FFFFFFFFFFFFFFFF]    [1..64] 10^19
...
typedef struct { anu l, m[254], h; } MatBuf;                    // 256                            + [2041..2048] 10^616 v{10^1228}
