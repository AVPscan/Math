...
* **Нет типов** Зачем разделять `256/128/64/32/16/8` какой в этом смысл? Менее `33 атомов` этого так же не достаточно и зачем себя ограничивать...
* **Нет предела**: Существует мнение что число атомов во вселенно примерно 10^80 `266 бит = 34 атома` теперь доступно `256 атомов = 2048 бит`, но настоятельно рекомендую ограничиться `128 атомами = 1024 бита` чтоб умножая такие числа не вылетать в переполнение ибо это не имеет смысла.
