...
* **Процессор нативно работает с состояниями**: Конвейер никогда не стопится, не сбрасывает такты и не падает в аппаратные прерывания.
* **Нет типов** Зачем разделять 64/32/16/8 какой в этом смысл? 128 бит = 16 атомов этого так же не достаточно и зачем себя ограничивать...
* **Нет предела**: Существует мнение что число атомов во вселенно примерно 10^80 это 266 бит = 34 атомов, теперь доступно 256 но настоятельно рекомендую ограничиться 128 атомами чтоб умножая такие числа не вылетать в переполнение ибо это не имеет смысла.
